...
BUCKET_NAME = os.environ.get("DIFF_CACHE_BUCKET", "")
...
def get_cached_diff(repo: str, pr: int, sha: str) -> str | None:
    ...
    try:
        ...
        response = client.get_object(Bucket=BUCKET_NAME, Key=_diff_key(repo, pr, sha))
...
def get_cached_analysis(repo: str, pr: int, sha: str) -> list[Finding] | None:
    ...
    try:
        client = get_s3_client()
        response = client.get_object(Bucket=BUCKET_NAME, Key=_analysis_key(repo, pr, sha))
